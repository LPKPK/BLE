...
/* Private variables ---------------------------------------------------------*/
static const uint8_t SizeHrme = 23;
static const uint8_t SizeBsl = 1;
static const uint8_t SizeHrcp = 1;
...
void HRS_Init(void)
{
  ...
  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail   : aci_gatt_add_char command   : HRME, error code: 0x%2X\n", ret);
  }
  else
  {
  ...
  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail   : aci_gatt_add_char command   : BSL, error code: 0x%2X\n", ret);
  }
  else
  {
  ...
  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail   : aci_gatt_add_char command   : HRCP, error code: 0x%2X\n", ret);
  }
  else
  {
...
tBleStatus HRS_UpdateValue(HRS_CharOpcode_t CharOpcode, HRS_Data_t *pData)
{
  ...
  switch(CharOpcode)
  {
    case HRS_HRME:
      ...
      if (ret != BLE_STATUS_SUCCESS)
      {
        APP_DBG_MSG("  Fail   : aci_gatt_update_char_value HRME command, error code: 0x%2X\n", ret);
      }
      else
      {
        APP_DBG_MSG("  Success: aci_gatt_update_char_value HRME command\n");
      }
    ...
    case HRS_BSL:
      ...
      if (ret != BLE_STATUS_SUCCESS)
      {
        APP_DBG_MSG("  Fail   : aci_gatt_update_char_value BSL command, error code: 0x%2X\n", ret);
      }
      else
      {
        APP_DBG_MSG("  Success: aci_gatt_update_char_value BSL command\n");
      }
    ...
    case HRS_HRCP:
      ...
      if (ret != BLE_STATUS_SUCCESS)
      {
        APP_DBG_MSG("  Fail   : aci_gatt_update_char_value HRCP command, error code: 0x%2X\n", ret);
      }
      else
      {
        APP_DBG_MSG("  Success: aci_gatt_update_char_value HRCP command\n");
      }
