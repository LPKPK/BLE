...
static const uint8_t SizeMons = 32;
...
void DIS_Init(void)
{
  ...
  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail   : aci_gatt_add_char command   : MONS, error code: 0x%2X\n", ret);
  }
  else
  {
...
tBleStatus DIS_UpdateValue(DIS_CharOpcode_t CharOpcode, DIS_Data_t *pData)
{
  ...
  switch(CharOpcode)
  {
    ...
    case DIS_MONS:
      ...
      if (ret != BLE_STATUS_SUCCESS)
      {
        APP_DBG_MSG("  Fail   : aci_gatt_update_char_value MONS command, error code: 0x%2X\n", ret);
      }
      else
      {
